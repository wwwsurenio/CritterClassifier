        ...
        let request = VNCoreMLRequest(model: model) { (request, error) in
            ...
            if let firstResult = results.first{
                if firstResult.identifier.contains("Dog"){
                    ...
                    self.textBox.text = "This looks like a Dog, our classifier is confident with \(String(format: "%.0f", (firstResult.confidence * 100))) % "
                    ...
                    self.textBox.text = "This looks like a Cat, our classifier is confident with \(String(format: "%.0f", (firstResult.confidence * 100))) % "
                    self.textBox.sizeToFit()
                    ...
                    self.textBox.text = "This looks like a Rabbit, our classifier is confident with \(String(format: "%.0f", (firstResult.confidence * 100))) % "
                    self.textBox.sizeToFit()
                }else{
